...
namespace CoreMVC.AutomaticTokenManagement
{
    public class AutomaticTokenManagementCookieEvents : CookieAuthenticationEvents
    {
        ...
        public override async Task ValidatePrincipal(CookieValidatePrincipalContext context)
        {
            ...
            if (dtRefresh < _clock.UtcNow.AddSeconds(50))
            {
                ...
                if (shouldRefresh)
                {
                    try
                    {
                        var response = await _service.RefreshTokenAsync(refreshToken.Value.Substring(2));
